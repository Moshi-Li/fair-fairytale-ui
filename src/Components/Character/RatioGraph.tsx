...
const RatioGraph = () => {
  ...
  const result: any[] = Object.keys(dataReducer.characterMeta).map((key) => {
    const { easyName, total } =
      dataReducer.characterMeta[key];
  ...
  var trace1: Plotly.Data = {
    x: result.map(item=>{return item.appearance}),
    transforms: [{
        type: 'sort',
        target: 'x',
        order: 'ascending'
    ...
    y: result.map(item=>{return item.name}),
    type: "bar",
    text: result.map(item=>{return item.appearance}),
  ...
  var layout = {
    ...
    "xaxis": {
      "visible": false,
      "range": [0,result.map(item=>{return item.appearance}).length-1],
    },
    "yaxis": {
        "showgrid": false,
        "zeroline": false,
        "range": [0,result.map(item=>{return item.name}).length-1]
    }
  };
